...
- the python binary has been configured with `cap_sys_ptrace` capability, meaning that it could be abused to debug processes and potentially inject shellcode
...
If we wanted to follow the traditional route, we could use this method of code execution to gain access to the `shaun` account, by inserting our ssh public key into `/home/shaun/.ssh/authorized_keys` (and creating the file if necessary). But since we're already able to execute code as root, we may as well take the same approach and put our key into `/root/.ssh/authorized_keys` with a neat one-liner that will ensure we can ssh in as root:
```
...
From our attack box, we can now log in as root and access both `user.txt` and `root.txt`:
```
┌──(kali㉿kali)-[~]
└─$ ssh root@doctor.htb 
...
root@doctor:~# cat ~shaun/user.txt
67c73***************************
