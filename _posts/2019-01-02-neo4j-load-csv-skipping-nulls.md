...
Essentially the *unwind range(1, 4)* allows us to iterate through the columns of a single row, and take any necessary action on each value. Null values are skipped throughout the row by testing their ability to match an existing node, but the logic would be similar for actual non-null (*row[i] <> ""*) evaluation.
